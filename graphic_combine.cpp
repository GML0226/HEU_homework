...
// 绘制坐标轴和刻度
void drawAxes(int offsetX, int offsetY, int width, int height) {
...
// 绘制曲线
void drawCurve(const vector<Point>& points, int offsetX, int offsetY, int width, int height, COLORREF color) {
    if (points.empty()) {
...
// 在每个曲线下方绘制文字标注
void drawLabel(int offsetX, int offsetY, int width, const string& label, double auc, COLORREF color) {
...
void drawBarChart(const vector<vector<double>>& results, int offsetX, int offsetY, int width, int height) {
    ...
    // 定义颜色数组
    int colors[] = { LIGHTMAGENTA, LIGHTGREEN, YELLOW, LIGHTCYAN, LIGHTRED };
...
int main() {
    ...
    // 绘制坐标轴
    int margin = 50;
    int graphWidth = winWidth - 2 * margin;
    int graphHeight = winHeight - 2 * margin;
    drawAxes(margin, winHeight - margin, graphWidth, graphHeight);

    // 绘制五条曲线
    drawCurve(points1, margin, winHeight - margin, graphWidth, graphHeight, LIGHTMAGENTA);
    drawLabel(margin, winHeight - margin - 300, graphWidth, "bpNET", auc1, LIGHTMAGENTA);

    drawCurve(points2, margin, winHeight - margin, graphWidth, graphHeight, LIGHTGREEN);
    drawLabel(margin, winHeight - margin - 250, graphWidth, "KNN", auc2, LIGHTGREEN);

    drawCurve(points3, margin, winHeight - margin, graphWidth, graphHeight, YELLOW);
    drawLabel(margin, winHeight - margin - 200, graphWidth, "Logic", auc3, YELLOW);

    drawCurve(points4, margin, winHeight - margin, graphWidth, graphHeight, LIGHTCYAN);
    drawLabel(margin, winHeight - margin - 150, graphWidth, "RTrees", auc4, LIGHTCYAN);

    drawCurve(points5, margin, winHeight - margin, graphWidth, graphHeight, LIGHTRED);
    drawLabel(margin, winHeight - margin - 100, graphWidth, "SVM", auc5, LIGHTRED);
